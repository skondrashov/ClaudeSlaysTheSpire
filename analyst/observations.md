...
- **Boss relic collection SKIPPED (3 occurrences: Run 71, Run 77, Run 142).** In Run 142, after Slime Boss victory, two consecutive `proceed` commands (lines 162-163) bypassed the BOSS_REWARD screen entirely. No boss relic was selected or collected. The player's reasoning said "Proceed to boss relic rewards" followed by "Open boss treasure chest" but no `choose` command occurred between them. This is now confirmed across 3 separate runs. **CRITICAL BUG -- boss relics are among the strongest items in the game. Skipping them is equivalent to a free relic lost every run.** Needs immediate infrastructure fix.
- **Treasure chest relic SKIPPED (1 occurrence: Run 142 Floor 9).** After entering Treasure room (line 75), a single `proceed` command (line 76) bypassed the chest contents. No relic was collected -- the next action was choosing the next map room. This is a NEW bug type, distinct from the boss relic skip. The Act 2 chest at F5 was eventually collected (Ceramic Fish) but required multiple attempts (lines 237-240). The chest skip bug may be related to the boss relic skip bug -- both involve `proceed` bypassing reward screens.
...
- **Boss relic collection skipped** (Run 71, Run 77, Run 142): Automation bug. 3 confirmed occurrences. STILL UNRESOLVED. Needs immediate infrastructure investigation. Run 142 also showed a treasure chest skip on Floor 9 (same `proceed` bypass mechanism).
