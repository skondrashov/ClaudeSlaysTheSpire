...
- Chosen 7x2 damage: Confirmed 7x2 base damage in Run 20 (dealt 20 with Vulnerable on player: 7*1.5=10.5 rounded to 10, times 2 = 20). The Vulnerable application makes this attack extremely lethal. Does Chosen always apply Vulnerable before the 7x2 attack, or is it a later pattern?
...
- Index shifting persists as fatal error: Despite player.md explicitly saying "Prefer card names over indices," the player used numeric indices 100% of the time in the Run 21 3-Cultists fight. Multiple plays resolved to wrong cards (Strike instead of Rampage, Strike instead of Defend, Strike instead of Fiend Fire). The documentation exists but is not being followed. Hypothesis: the player's turn-planning logic defaults to indices when reasoning about card positions. The fix may need to be in the system prompt phrasing, not just the playbook.
