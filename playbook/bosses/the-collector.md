...
SILENT POISON KILL CONFIRMED: Entered at 62/70 HP (88%). Catalyst+ tripled 5 poison to 15 on Turn 4 (STRONG_DEBUFF, free turn). Used Poison Potion, Cunning Potion, and Attack Potion on Turn 5 (crisis turn) while Piercing Wail reduced incoming by 6 Str per enemy. Neutralize+ for Weak. Survived post-debuff at 27 HP. Noxious Fumes+ (3 poison/turn) killed Torch Heads passively while poison on the Collector ticked for 30-40+ per turn. Fight lasted ~12 turns. Collector killed by Strike + Sucker Punch through 12 block when at 1 HP. Key: WLP+ retained Piercing Wail and Catalyst+ across turns until needed. TotT discarded Strikes (useless) to find block cards. The Normality curse (from Cursed Key) limited plays to 3/turn on 2 turns -- manageable because poison damage is automatic and does not require card plays.
