...
- **Play high-impact cards, minimize card count per turn.** Body Slam+ (0E, damage = block) is ideal -- one card, massive damage. Immolate+ (one card, 28+ AOE) is excellent. Avoid playing multiple small attacks when one big attack suffices.
- **Barricade is the best card in this fight.** Block persists through Time Warp turn resets. After Time Warp ends your turn, all accumulated block carries over. This means block invested before Time Warp is NOT wasted.
- **Entrench+ doubles block for Body Slam+ scaling.** One Entrench+ play followed by one Body Slam+ play = 2 cards for massive damage.
