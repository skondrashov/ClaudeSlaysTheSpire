...
STRATEGY:
- **Turn 1**: Block is mandatory. Do not set up powers. Flame Barrier+ (16 block + 6 counter to all 3 attackers = 18 counter) is the ideal turn 1 play if drawn. AOE is valuable for spreading damage.
- **Kill priority**: Focus fire on the enemy with the highest per-turn damage first. Reduce incoming ASAP.
- **Rage synergy**: Rage (0E, 3 block per Attack) is excellent because it generates free block while dealing damage.
