...
**Calibration = belief error.** If the run carries the player's own live eval trail (Casual Win and Explore runs do), compare its MOVES to your forward estimates. The trail is relative by design — the player starts at even and labels signed moves; the analyst owns absolutes, so pin the trail to the corpus base rate yourself when an absolute matters. A wrongly-sized or missing move is a place the player's *beliefs* were wrong, independent of whether they won — and it shows up in won runs too, so mine those as well. High regret with good calibration is an EXECUTION flag (knew better, didn't do it); a large calibration gap is a CURATOR flag (the playbook taught a wrong prior). A label re-affirmed unchanged across a material state change is a calibration finding even when every sized update is right.
