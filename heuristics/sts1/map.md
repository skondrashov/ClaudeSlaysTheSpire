...
**Priority order: ledger first, then default-aggressive.** Price each route first — the sum of its fights' expected HP costs against current HP and the route's recovery rooms (see the expected-damage ledger in [[layer:heuristics, hp-management]]). Routes whose ledger goes negative are off the table. Among routes that survive the arithmetic, default to the most elite-heavy one. These two doctrines are sequential, not in tension.
...
- **Re-route only when forced.** Follow your act plan. Deviate when a fight costs well over its expected price and the remaining ledger no longer clears.
- **At each floor, re-run the remaining ledger.** Sum the expected costs of the forced combats to the next rest site; if current HP doesn't cover them with margin for a spike, re-route.
...
**An elite is affordable when the ledger clears: current HP covers the elite's expected cost (see its page — typically 25-45, matchup-dependent) PLUS the expected costs of every forced fight between the elite and the next recovery, with margin for a spike.** A rest site within a floor or two of the elite makes most elites affordable; back-to-back elites with no rest between stack two full expected costs and usually don't clear.
...
1. **The ledger**: does current HP cover this elite's expected cost plus the rest of the path to recovery?
...
- **Book of Stabbing**: Double scaling — +1 hit/turn AND +3 Str/turn. Wounds clog draw pile. NEED: exhaust for Wounds, fast kill, and HP for its full expected cost (its double scaling makes slow decks pay multiples of it). **If you have Brimstone, DO NOT FIGHT. Brimstone + Book of Stabbing = unsurvivable.**
- **[[enemies/Gremlin Leader]]**: Rally gives +3 Str to ALL enemies per cast. Must win by turn 5-6 or Rally scaling overwhelms you (see [[layer:heuristics, enemies/Gremlin Leader]]). NEED: AOE for gremlins, [[cards/Shockwave]] for mass debuff, and HP for its full expected cost.
- **Slavers**: 3 enemies, 27+ combined damage from Turn 1. NEED: AOE, mass debuff, block density, and HP for its expected cost (see [[layer:heuristics, enemies/Slavers]]).
...
When holding Brimstone, price Act 2 elites at the top of their cost range or above — the ledger rarely affords them.
...
- **Rest over upgrade when HP doesn't cover the boss's expected cost** (the boss page states it; 30-50 is typical) — otherwise take the upgrade.
