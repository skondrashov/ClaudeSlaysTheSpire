...
| Function | Description |
|----------|-------------|
| `survey()` | Two parts. First, deterministic: the screen you're on IS a decision boundary (fight start, turn, card reward, shop, rest site, ...), and survey names its boundary index — `awareness/sts1/boundaries/<name>` — the considerations that apply to every decision of this kind. Second, selector judgment: one small-model call maps the live state → recall() handles worth pulling — the on-board entities (with their heuristic handles), upgraded cards, and any phenomenon whose conditions match right now. It surfaces the non-obvious (combos, contextual warnings). Discovery, not content. Use at meaningful decision points (combat start, rewards, before elites/bosses), not every action. |
| `recall(*handles)` | Fetch knowledge by handle. A handle is a wiki-style address (`"enemies/Gremlin Nob"`, `"layer:heuristics, cards/Bash"`), a bare name (`"Gremlin Nob"` → ontology), an upgraded card (`"Bash+"` → its resolved phenomenon), a heuristics topic (`"layer:heuristics, combat"` — also `map`, `hp-management`, `drafting`, `rest-sites`, `archetypes`), a boundary index (`"boundaries/turn"`), or a recognition (`"recognitions/potion-use"`). Names resolve robustly: variants (`"Spike Slime (M)"`), apostrophes (`"Charon's Ashes"`), a leading "The" added or removed (`"Champ"` → The Champ), `"Ascension N"` → its modifier entry, and map aliases (`"The Transient"`, `"Centurion + Mystic"`). The state always names the precise enemy variant (`"Red Louse"`, `"Blue Slaver"`) — recall that name, not a generic one. A bare name in several categories returns every match. Pass several handles for several entries (an entity's ontology AND heuristic = two handles). Does NOT follow links — `recall()` them too if you want them. **The single lookup verb.** |
